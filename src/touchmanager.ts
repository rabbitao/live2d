...
export class TouchManager {
    ...
    public _startY: number;            // タッチを開始した時のxの値
    public _startX: number;            // タッチを開始した時のyの値
    public _lastX: number;             // シングルタッチ時のxの値
    public _lastY: number;             // シングルタッチ時のyの値
    public _lastX1: number;            // ダブルタッチ時の一つ目のxの値
    public _lastY1: number;            // ダブルタッチ時の一つ目のyの値
    public _lastX2: number;            // ダブルタッチ時の二つ目のxの値
    public _lastY2: number;            // ダブルタッチ時の二つ目のyの値
    public _lastTouchDistance: number; // 2本以上でタッチしたときの指の距離
    public _deltaX: number;            // 前回の値から今回の値へのxの移動距離。
    public _deltaY: number;            // 前回の値から今回の値へのyの移動距離。
    public _scale: number;             // このフレームで掛け合わせる拡大率。拡大操作中以外は1。
    public _touchSingle: boolean;      // シングルタッチ時はtrue
    public _flipAvailable: boolean;    // フリップが有効かどうか
    /**
     * コンストラクタ
     */
    constructor() {
