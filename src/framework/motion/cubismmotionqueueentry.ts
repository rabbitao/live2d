...
export namespace Live2DCubismFramework {
    ...
    export class CubismMotionQueueEntry {
        ...
        public _autoDelete: boolean;               // 自動削除
        public _motion: ACubismMotion;             // モーション

        public _available: boolean;                // 有効化フラグ
        public _finished: boolean;                 // 終了フラグ
        public _started: boolean;                  // 開始フラグ
        public _startTimeSeconds: number;          // モーション再生開始時刻[秒]
        public _fadeInStartTimeSeconds: number;    // フェードイン開始時刻（ループの時は初回のみ）[秒]
        public _endTimeSeconds: number;            // 終了予定時刻[秒]
        public _stateTimeSeconds: number;          // 時刻の状態[秒]
        public _stateWeight: number;　             // 重みの状態
        public _lastEventCheckSeconds: number;     // 最終のMotion側のチェックした時間

        public _motionQueueEntryHandle: CubismMotionQueueEntryHandle; // インスタンスごとに一意の値を持つ識別番号
        /**
         * コンストラクタ
         */
        public constructor() {
