...
export namespace Live2DCubismFramework {
    ...
    export class CubismClippingManager_WebGL {
      ...
      public gl: WebGLRenderingContext = null as any;  // WebGLレンダリングコンテキスト
    ...
    export class CubismClippingContext {
      ...
      public _isUsing: boolean = undefined as any;  // 現在の描画状態でマスクの準備が必要ならtrue
        public readonly _clippingIdList: Int32Array;    // クリッピングマスクのIDリスト
        public _clippingIdCount: number;   // クリッピングマスクの数
      public _layoutChannelNo: number = undefined as any;  // RGBAのいずれのチャンネルにこのクリップを配置するか（0:R, 1:G, 2:B, 3:A）
    ...
    export class CubismShader_WebGL {
        ...
        public _shaderSets: csmVector<CubismShaderSet>; // ロードしたシェーダープログラムを保持する変数
      public gl: WebGLRenderingContext = undefined as any;  // webglコンテキスト
    ...
    export class CubismShaderSet {
      public shaderProgram: WebGLProgram = undefined as any;                         // シェーダープログラムのアドレス
      public attributePositionLocation: GLuint = undefined as any;                  // シェーダープログラムに渡す変数のアドレス（Position）
      public attributeTexCoordLocation: GLuint = undefined as any;                  // シェーダープログラムに渡す変数のアドレス（TexCoord）
      public uniformMatrixLocation: WebGLUniformLocation = undefined as any;        // シェーダープログラムに渡す変数のアドレス（Matrix）
      public uniformClipMatrixLocation: WebGLUniformLocation = undefined as any;    // シェーダープログラムに渡す変数のアドレス（ClipMatrix）
      public samplerTexture0Location: WebGLUniformLocation = undefined as any;      // シェーダープログラムに渡す変数のアドレス（Texture0）
      public samplerTexture1Location: WebGLUniformLocation = undefined as any;      // シェーダープログラムに渡す変数のアドレス（Texture1）
      public uniformBaseColorLocation: WebGLUniformLocation = undefined as any;     // シェーダープログラムに渡す変数のアドレス（BaseColor）
      public uniformChannelFlagLocation: WebGLUniformLocation = undefined as any;   // シェーダープログラムに渡す変数のアドレス（ChannelFlag）
    }
    ...
    export class CubismRenderer_WebGL extends CubismRenderer {
        ...
        public _textures: csmMap<number, WebGLTexture>;                      // モデルが参照するテクスチャとレンダラでバインドしているテクスチャとのマップ
        public _sortedDrawableIndexList: csmVector<number>;             // 描画オブジェクトのインデックスを描画順に並べたリスト
        public _clippingManager: CubismClippingManager_WebGL;          // クリッピングマスク管理オブジェクト
        public _clippingContextBufferForMask: CubismClippingContext;   // マスクテクスチャに描画するためのクリッピングコンテキスト
        public _clippingContextBufferForDraw: CubismClippingContext;   // 画面上描画するためのクリッピングコンテキスト
